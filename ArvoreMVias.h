...
const int MAX_DATA_SIZE = 120;    // Tamanho Maximo dos Dados Associados a Chave (Registros de Tamanho Fixo)
...
struct Registro {
    ...
    char dados[MAX_DATA_SIZE];    // Informacao Associdada a Chave
...
private:
    ...
    int M;                    // Ordem da Arvore (Maximo de Filhos)
    ...
    void splitChild(int parentId, int childIndex, int childId); // Divide o Noh quando um Noh Filho estah Cheio.
...
public:
    ...
    void insertB(int chave, const string& dadosElemento);    // Insere Chave e Dados.
